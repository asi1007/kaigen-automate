"""輸入許可書から経理を作成するユースケース"""
import logging
from pathlib import Path

from src.domain.entities.import_permit import ImportPermit
from src.domain.repositories.moneyforward_repository import IMoneyforwardRepository
from src.infrastructure.pdf_parser.import_permit_parser import ImportPermitParser

logger = logging.getLogger(__name__)


class CreateAccountingFromImportPermitUseCase:
    """輸入許可書PDFからマネーフォワードの経理を作成するユースケース"""

    def __init__(
        self,
        import_permit_parser: ImportPermitParser,
        moneyforward_repository: IMoneyforwardRepository,
    ):
        self.import_permit_parser = import_permit_parser
        self.moneyforward_repository = moneyforward_repository

    async def execute(self, pdf_path: Path) -> str:
        """輸入許可書PDFから経理を作成する

        Args:
            pdf_path: 輸入許可書PDFファイルのパス

        Returns:
            str: 作成された経理のID

        Raises:
            ValueError: PDFの解析に失敗した場合
            Exception: 経理作成に失敗した場合
        """
        logger.info(f"輸入許可書から経理を作成する処理を開始します: {pdf_path}")

        try:
            # ステップ1: PDFを解析してImportPermitエンティティに変換
            logger.info("ステップ1: 輸入許可書PDFを解析中...")
            import_permit = self.import_permit_parser.parse(pdf_path)
            logger.info(
                f"輸入許可書の解析が完了しました: {import_permit.permit_number} "
                f"(金額: ¥{import_permit.total_amount:,})"
            )

            # ステップ2: マネーフォワードに経理を登録
            logger.info("ステップ2: マネーフォワードに経理を登録中...")
            transaction_id = await self.moneyforward_repository.create_transaction_from_import_permit(import_permit)
            logger.info(f"経理の登録が完了しました: {transaction_id}")

            logger.info(
                f"処理完了: 輸入許可書 {import_permit.permit_number} から経理 {transaction_id} を作成しました"
            )
            return transaction_id

        except ValueError as e:
            logger.error(f"輸入許可書の解析に失敗しました: {e}")
            raise
        except Exception as e:
            logger.error(f"経理作成中にエラーが発生しました: {e}")
            raise

